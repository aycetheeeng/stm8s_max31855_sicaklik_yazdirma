...
/* Includes ------------------------------------------------------------------*/
#include "stm8s_it.h"
extern void sayiyi_yazdir(int x);
extern int x;
...
INTERRUPT_HANDLER(TIM4_UPD_OVF_IRQHandler, 23)
{
 ...
 if (TIM4_GetITStatus(TIM4_IT_UPDATE) == SET){
	...
	if(i <= 1){
	//birler basamaðý
		    GPIO_WriteHigh(LEDGRP2_PORT,LEDGRP2_PIN);
				GPIO_WriteLow(LEDGRP0_PORT,LEDGRP0_PIN);
		    GPIO_WriteLow(LEDGRP1_PORT,LEDGRP1_PIN);
		    GPIO_WriteLow(LEDGRP3_PORT,LEDGRP3_PIN);
		    sayiyi_yazdir(DispBuff[0]);
	}else if(i <= 2){
	//onlar basamaðý
        GPIO_WriteHigh(LEDGRP0_PORT,LEDGRP0_PIN);
				GPIO_WriteLow(LEDGRP2_PORT,LEDGRP2_PIN);
		    GPIO_WriteLow(LEDGRP1_PORT,LEDGRP1_PIN);
		    GPIO_WriteLow(LEDGRP3_PORT,LEDGRP3_PIN);
		    sayiyi_yazdir(DispBuff[1]);
	}else if(i <= 3){
	//yüzler basamaðý	
        GPIO_WriteHigh(LEDGRP1_PORT,LEDGRP1_PIN);
				GPIO_WriteLow(LEDGRP0_PORT,LEDGRP0_PIN);
		    GPIO_WriteLow(LEDGRP2_PORT,LEDGRP2_PIN);
		    GPIO_WriteLow(LEDGRP3_PORT,LEDGRP3_PIN);
		    sayiyi_yazdir(DispBuff[2]);
	}else if(i <= 4){
	//binler basamaðý		
			  GPIO_WriteHigh(LEDGRP3_PORT,LEDGRP3_PIN);
				GPIO_WriteLow(LEDGRP0_PORT,LEDGRP0_PIN);
		    GPIO_WriteLow(LEDGRP1_PORT,LEDGRP1_PIN);
	    	GPIO_WriteLow(LEDGRP2_PORT,LEDGRP2_PIN);
		    sayiyi_yazdir(DispBuff[3]);
